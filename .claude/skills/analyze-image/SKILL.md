---
name: analyze-image
description: 分析本地截图/图片（报错、界面、文字）。当用户要求"分析这张截图/图片""分析最新一张截图""看下这个报错""看看这个界面"，或用户直接粘贴了一张截图/图片（对话里出现 [Image] 引用）时使用。
---
...
3. 用户**在 VS Code 插件面板里粘贴**了图片（对话里有 `[Image]` 引用、但**没有** `source:` 路径——面板粘贴的图以 base64 内嵌在会话转录里，**不落盘**）→ **一律走这一步**：运行 `node extract-pasted-image.mjs`，按返回结果处理：
   - 输出**一个路径** → 这是最新的一张粘贴图，用该路径作为 `path` 分析。
   - 输出 **`SAME:<路径>`** → 转录里最新的图就是已提取过的那张（**本次没有新粘贴**）。重新分析该路径，或告诉用户"没检测到新的粘贴，这是最近一次粘贴的图"。
...
- `task`：`error`=报错定位 / `ui`=界面走查 / `ocr`=纯文字提取 / `general`=全面分析（默认）
- `lang`：`zh`（默认）或 `en`
