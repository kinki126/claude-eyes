...
**若返回里有 `boxed_text`（框选/高亮区域内文字的逐字转录）**：这是用户最关注的内容，务必**单独醒目地呈现**，并把它作为分析/定位的核心线索。

## 3. 自动定位到项目代码（可选增强）

返回的 `keywords` 是可用于代码检索的关键词（报错函数名 / 错误码 / 文件名 / 接口路径等）；`boxed_text` 是用户框选区域的逐字转录，**优先用它**。当它们非空、且当前是代码项目时：
- 在当前项目里用 grep/搜索这些关键词（优先函数名、文件名、错误码、接口路径，优先 `boxed_text` 里的内容）。
