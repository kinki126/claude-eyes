...
server.tool(
    ...
    {
        path: z.string().describe('本地图片绝对路径，Windows 格式，如 E:\\temp\\shot.png'),
        description: z.string().optional().describe('用户的文字描述，透传给视觉模型，可省略'),
        task: z.enum(['error', 'ui', 'ocr', 'general']).optional().default('general')
            .describe('分析任务类型：error=报错定位 ui=界面走查 ocr=纯文字提取 general=全面分析'),
