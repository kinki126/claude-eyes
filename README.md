...
- 🧾 **Structured diff output** (v1.6.0) — `task=diff` returns a `diffs[]` array listing each change with `change_type` (add/remove/modify) and normalized `bbox`, so Claude can render a markdown table of differences instead of dumping prose
- ✅ **Assertion-style verify** (v1.6.0) — `task=verify` accepts `description: "assert=按钮是否为红色"` and returns a structured `verify: { passed: true|false|null, reason }` (aligned with `verdict`; `uncertain` forces `passed=null`) for screenshot regression scenarios
- 🗂️ **History search** (v1.6.0) — `GET /history?task=&keyword=&since=&until=&limit=` endpoint + `search_history` MCP tool search the persisted `.claude-eyes/history.jsonl` by task/keyword/time range (replaces manual grep)
- 🔄 **Multi-turn context inheritance** (v1.5.0) — bridge caches the last analysis context per image md5; same-image follow-ups with `focus`/`crop_bbox` auto-inject the previous `{ task, keywords, regions, analysis summary }` so you don't repeat "the screenshot from just now"
- 📢 **Stage progress notifications** (v1.5.0) — `analyze_image` / `locate_code` / `search_history` handlers push `notifications/message` at key stages (preparing → bridge ready → calling model → done), reducing perceived latency in MCP clients
- 🔍 **Code locator** (v1.5.0, v1.6.1 Node-native) — `locate_code` MCP tool searches project code with **Node-native recursive walk** (no external rg/findstr/grep dependency, cross-platform stable) and returns `{ file, line, match }` candidates with **definition-first ordering** (function/const/class declarations rank above comment/import references)
