# Claude Code Vision Bridge

让 **Claude Code**(后端无视觉能力,如 DeepSeek)通过**函数调用**分析本地截图/图片:用户在对话里粘贴图片或给出路径,Claude 调用 `analyze_image` 工具,视觉模型分析后返回结构化结果,并**自动决定**本次分析流程是继续(等下一张)还是停止。

> 适用场景:你的 Claude Code 用的是不支持多模态的后端(DeepSeek 等),但需要分析截图/报错/界面时。

## 特性

- 🖼️ **函数调用分析图片**:Claude 原生调用 `analyze_image` MCP 工具,无需换模型
- 🔁 **自动停/续控制**:视觉模型根据图片内容自主决定 `continue`(继续等下一张)或 `stop`(流程结束)
- 📋 **三种喂图方式**:VS Code 面板粘贴(Trae 式)/ 截图自动保存 / 直接给路径
- 🔌 **可换提供商**:只依赖 OpenAI 兼容的 chat/completions + vision 接口,改配置即可切换智谱/OpenAI/通义千问-VL/DeepSeek-VL/Ollama/vLLM
...
```
Claude Code ──函数调用──▶ mcp-image-analyzer (MCP) ──HTTP──▶ zhipu-bridge-api (8765) ──▶ 视觉LLM
                              ▲                                     │
                        ensureBridge 自动拉起                统一响应 { analysis, control }
```
...
| 方式 | 操作 |
|---|---|
| 面板粘贴(Trae 式) | VS Code 插件面板 `Ctrl+V` 图片 → 回车,自动分析 |
...
配置优先级:环境变量 > `vision-config.json` > 默认值。换模型、多模型回退、缓存/限流/日志等详见 [`docs/CONFIG.md`](docs/CONFIG.md)。
...
```bash
npm run smoke                      # 本地 MCP 冒烟
npm run smoke:remote               # 远程 HTTP MCP 冒烟
curl http://127.0.0.1:8765/health  # 桥接健康检查
```
