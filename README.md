...
- 🔍 **Code locator** (v1.5.0) — `locate_code` MCP tool searches project code (ripgrep / findstr / grep fallback) and returns `{ file, line, match }` candidates so Claude can jump from screenshot error to source line
